...
class LeCartCustom(LeBasecart):
    ...
    def get_categories_ext_export(self, categories):
        ...
        categories_ext_queries = {
            ...
            'languages': {
                'type': 'select',
                'query': "SELECT * FROM categories_description as de inner join  languages as la on "+
                         " de.language_id = la.languages_id WHERE de.categories_id IN " + self.list_to_in_condition(
        ...
        # self.log(categories_ext,'categories_ext')
    ...
    def get_products_main_export(self):
        ...
        query = {
            'type': 'select',
            'query': "SELECT * FROM  _DBPRF_products WHERE products_id > " + to_str(id_src) + " ORDER BY products_id ASC LIMIT " + to_str(
                limit)
        }
    ...
    def get_products_ext_export(self, products):
        ...
        product_ext_queries = {
            ...
            'attribute': {
                'type': "select",
                'query': "SELECT * FROM _DBPRF_attribute as at JOIN _DBPRF_product_attribute pa ON pa.attribute_id = at.attribute_id WHERE pa.product_id IN " + product_id_con
            }
        }
    ...
    def convert_product_export(self, product, products_ext):
        ...
        product_attribute = get_list_from_list_by_field(products_ext_data['attribute'], 'product_id',
                                                         product['products_id'])
        if product_attribute:
            for product_attribute in product_attribute:
                product_attribute_data = self.construct_product_attribute()
                product_attribute_data['option_id'] = product_attribute['attribute_id']
                ...
                product_attribute_data['option_type'] = product_attribute['attribute_code']
    ...
    def get_customers_ext_export(self, customers):
        ...
        # self.log(customers_ext,'customers_exts')
    ...
    def convert_customer_export(self, customer, customers_ext):
        ...
        customers_infos = get_list_from_list_by_field(customers_ext['data']['customers_info'], 'customers_info_id',
                                                    customer['customers_id'])
        # self.log(customers_infos,'customers_infos')
        if customers_infos:
            for customers_info in customers_infos:
    ...
    def get_orders_ext_export(self, orders):
        ...
        if not orders_ext or orders_ext['result'] != 'success':
            return response_error()
        # self.log(orders_ext,'orders_extaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
        return orders_ext
    ...
    def convert_order_export(self, order, orders_ext):
        ...
        order_data['payment']['title'] = order['billing_name']


        order_customer = self.construct_order_customer()

        order_customer['id'] = order['client_customers_id']
        order_customer['email'] = '' if order['customers_email'] is None else order['customers_email']
        order_customer['middle_name']= order['billing_name']
        ...
        customer_address['address_1'] = '' if order['billing_address1'] is None else order['billing_address1']
        customer_address['address_2'] = order['billing_address2']
        customer_address['city'] = order['billing_city']
        customer_address['postcode'] = order['billing_postcode']
        customer_address['telephone'] = order['customers_telephone']
        customer_address['company'] = '' if order['billing_company'] is None else order['billing_company']
        ...
        order_billing['middle_name'] = fullName
        order_billing['last_name'] = newName[-1]
        newName.pop()
        order_billing['first_name'] = " ".join(newName)


        order_data['billing_address'] = order_billing

        order_delivery = self.construct_order_address()
        order_delivery['name'] = order['delivery_name']
        ...
        # orders_totals = get_list_from_list_by_field(orders_ext['data']['orders_total'], 'orders_id',
        #                                              order['orders_id'])
        # order_items_total = list()
        # for order_total in orders_totals:
        #     order_items_total = order_total
        # order_data['items'] = order_items_total
