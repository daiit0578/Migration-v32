...
class LeCartWoocommerce(LeCartWordpress):
    ...
    def clear_target_categories(self):
        ...
        while self._check_categories_exists():
            ...
            all_query_cat = {
                'clear_term': {
                    'type': 'query',
                    'query': "DELETE FROM `_DBPRF_terms` WHERE term_id IN " + self.list_to_in_condition(term_ids)
                },
                'clear_post': {
                    'type': 'query',
                    'query': "DELETE FROM `_DBPRF_posts` WHERE to_ping IN('TYPE_CATEGORY')"
                }
            }
            self.query_multiple_data_connector(all_query_cat)
    ...
    def clear_target_products(self):
        ...
        while self._check_product_exists():
            ...
            all_collections_query = {
                ...
                'type': 'query',
                'query': "DELETE FROM `_DBPRF_posts` WHERE post_type IN('product', 'revision') OR to_ping IN('TYPE_PRODUCT')"
            }
            clear_table = self.get_connector_data(self.get_connector_url('query'),
                                                  {'query': json.dumps(all_collections_query)})
            if (not clear_table) or (clear_table['result'] != 'success') or (not clear_table['data']):
            ...
            all_meta_query = {
                ...
                'clear_product_meta_lookup':{
                    'type': 'query',
                    'query': "DELETE FROM `_DBPRF_wc_product_meta_lookup` WHERE product_id NOT IN (SELECT ID FROM _DBPRF_posts)"
                },
                'clear_term_relationships': {
                    'type': 'query',
                    'query': "DELETE FROM `_DBPRF_term_relationships` WHERE object_id NOT IN (SELECT ID FROM _DBPRF_posts)"
                }
            }
            clear_table = self.query_multiple_data_connector(all_meta_query)
            # self.log(clear_table,'clear_tableaaaaaaaaaaaaaaaaaaaaaaaaa')
            if (not clear_table) or (clear_table['result'] != 'success'):
                self.log("Clear data failed. Error: Could not empty products", 'clear')
                continue
        # clear attributes
    ...
    def clear_target_orders(self):
        ...
        while self._check_order_exists():
            ...
            clear_table = self.get_connector_data(self.get_connector_url('query'),
                                                  {'query': json.dumps(clear_post_meta)})

            clear_woocommerce_order_items = {
                 'type': 'query',
                'query': "DELETE FROM `_DBPRF_woocommerce_order_items` WHERE order_id NOT IN (SELECT ID FROM _DBPRF_posts)"
            }
            clear_table = self.get_connector_data(self.get_connector_url('query'),
                                                  {'query': json.dumps(clear_woocommerce_order_items)})

            clear_woocommerce_order_itemmeta = {
                'type': 'query',
                'query': "DELETE FROM `_DBPRF_woocommerce_order_itemmeta` WHERE order_item_id NOT IN (SELECT order_item_id FROM _DBPRF_woocommerce_order_items)"
            }

            clear_table = self.get_connector_data(self.get_connector_url('query'),
                                                  {'query': json.dumps(clear_woocommerce_order_itemmeta)})
    ...
    def before_category_import(self, convert, category, categories_ext):
        return response_success()
    #TODO: CATEGORIES_IMPORT
    def category_import(self, convert, category, categories_ext):
        data_term ={
            'name' : convert['name'],
            'slug' : "-".join((convert['name'].lower()).split(' ')),
            'term_group': 0,
        }
        categories_query = self.create_insert_query_connector('terms', data_term)
        category_term_import = self.import_category_data_connector(categories_query, True, convert['id'])
        self.insert_map(self.TYPE_CATEGORY, convert['id'], category_term_import, convert['code'])
    ...
    def after_category_import(self, category_id, convert, category, categories_ext):
        parent = self.get_map_field_by_src(self.TYPE_CATEGORY, category['parent_id'], convert['code'])
        ...
        # self.log(term_taxonomy_data,'term_taxonomy_data_aaaaaaaaaaaaaaaaaaaaaaaaaaa')
        term_taxonomy_id  = self.import_category_data_connector(self.create_insert_query_connector("term_taxonomy", term_taxonomy_data), True,
                                           convert['id'])
        self.insert_map(self.TYPE_CATEGORY_TAXONOMY, convert['id'], term_taxonomy_id, convert['code'])
        termmeta_insert_order = {
            'term_id':category_id,
            'meta_key': 'order',
            'meta_value': category['sort_order'],
        }
        self.import_category_data_connector(self.create_insert_query_connector("termmeta", termmeta_insert_order),
                                            True,convert['id'])
        thumbnail_id = False
        cate_image = ''
        if convert['thumb_image']['url'] or convert['thumb_image']['path']:
            ...
            image_import_path = self.uploadImageConnector(image_process, self.add_prefix_path(
                self.make_woocommerce_image_path(image_process['path'], self.TYPE_CATEGORY),
                self._notice['target']['config']['image_category'].rstrip('/')))

            if image_import_path:
                cate_image = self.remove_prefix_path(image_import_path,
                                                     self._notice['target']['config']['image_category'])
                image_details = 'TYPE_CATEGORY'
                # self.log(cate_image, 'cate_image_cate')
                # self.log(image_details, 'image_details_cate')
                thumbnail_id = self.wp_image(cate_image, image_details)
        termmeta_insert_thumb = {
            'term_id': category_id,
            'meta_key': 'thumbnail_id',
            'meta_value': thumbnail_id
        }
        # self.log(termmeta_insert_thumb,'termmeta_insert_thumb')
        meta_query = self.create_insert_query_connector('termmeta', termmeta_insert_thumb)
        self.import_data_connector(meta_query, 'category')
        ...
        # term_relationship_data = {
        #     'object_id' : self.get_map_field_by_src(self.TYPE_PRODUCT, convert['id'], convert['code']),
        #     'term_taxonomy_id': term_taxonomy_id,
        #     'term_order': 0
        # }
        # meta_query = self.create_insert_query_connector('term_relationships', term_relationship_data)
        # self.import_data_connector(meta_query, 'category')
        # term_relationship_data = {
        #     'object_id': thumbnail_id,
        #     'term_taxonomy_id': 2,
        #     'term_order': 0
        # }
        # meta_query = self.create_insert_query_connector('term_relationships', term_relationship_data)
    ...
    def check_product_import(self, convert, product, products_ext):
        ...
        attribute = {
            'color' :{
                'name' : convert['attributes'][0]['option_name'],
                'value': convert['attributes'][0]['option_value_name'],
                'position': 0,
                'is_visible': 1,
                'is_variation': 0,
                'is_taxonomy': 0,
            ...
            'size' : {
                'name': convert['attributes'][1]['option_name'],
                'value': convert['attributes'][1]['option_value_name'],
                'position': 1,
                'is_visible': 1,
                'is_variation': 0,
                'is_taxonomy': 0,
            ...
            'sample' : {
                'name': convert['attributes'][2]['option_name'],
                'value': convert['attributes'][2]['option_value_name'],
                'position': 2,
                'is_visible': 1,
                'is_variation': 0,
                'is_taxonomy': 0,
        ...
        attribute_serialized = php_serialize(attribute)
        postmeta_data_update = {
            '_wc_average_rating':0  ,
            '_wc_review_count':0,
            '_product_attributes':attribute_serialized,
            '_product_version': '4.8.0',
            '_edit_last' :1
        }
        for key, value in postmeta_data_update.items():
            postmeta_data = {
                'post_id': product_id,
                'meta_key':key,
                'meta_value': value
            }
            postmeta_update_query = self.create_insert_query_connector('postmeta', postmeta_data)
            postmeta_update_id = self.import_order_data_connector(postmeta_update_query, True, convert['id'])
            # self.log(postmeta_update_id,'postmeta_update_idaaaaaaaaaaa')
        return product_id
    def update_latest_data_product(self, product_id, convert, product, products_ext):
        ...
        # begin
        product_query = self.create_update_query_connector("posts", {'ID': product_id, 'post_title': convert['name'],'ping_status':'closed'},
                                                           {'ID': product_id})
    ...
    def product_import(self, convert, product, products_ext):
        data = {
            ...
            'post_type': 'product',
            'comment_count': 0,
            'guid': convert['thumb_image']['url'] + convert['thumb_image']['path'],
            'post_excerpt': '',
            'to_ping': '',
            'pinged': '',
        ...
        product_query = self.create_insert_query_connector('posts', data)
        product_import = self.import_product_data_connector(product_query, True, convert['id'])

        self.insert_map(self.TYPE_PRODUCT, convert['id'], product_import, convert['code'])

        for term_relationship in products_ext['data']['products_to_categories']:
            if term_relationship['products_id']  == convert['id'] :

                term_taxonimy_id = self.get_map_field_by_src(self.TYPE_CATEGORY_TAXONOMY, term_relationship['categories_id'], convert['code'])
                term_relationship_data = {
                    'object_id': self.get_map_field_by_src(self.TYPE_PRODUCT, convert['id'], convert['code']),
                    'term_taxonomy_id': term_taxonimy_id if term_taxonimy_id else 1,
                    'term_order': 0
                }

                meta_query = self.create_insert_query_connector('term_relationships', term_relationship_data)
                self.import_data_connector(meta_query, 'product')
        # self.get_map_field_by_src(self.TYPE_PRODUCT, convert['id'], convert['code'])
        # self.log(get_map, 'get_map_ne')
        return response_success(product_import)

    def after_product_import(self, product_id, convert, product, products_ext):

        postMeta_data = {
            '_edit_lock' : int(time.time()),
            '_sku' : convert['sku'],
            '_regular_price' : product['products_msrp'],
            '_sale_price':product['products_wholesale_price'],
            '_price': product['products_wholesale_price'],
            'total_sales':0,
            '_tax_status': 'taxable',
            '_tax_class': '',
            '_manage_stock': 'no' if convert['sku'] is None else 'yes' ,
            '_backorders': 'no',
            ...
            '_download_expiry': -1,
            '_stock': convert['qty'],
            '_stock_status': 'instock' if int(convert['qty']) > 0  else 'outofstock'
        }
        for meta_key, value in postMeta_data.items():
            postmeta_data = {
        				'post_id': product_id,
        				'meta_key': meta_key,
        				'meta_value': value
        			}
            self.import_product_data_connector(self.create_insert_query_connector("postmeta", postmeta_data), True, convert['id'])
        product_meta_lookup ={
            'product_id' : product_id,
            'sku': convert['sku'],
            'min_price': product['products_wholesale_price'],
            'max_price': product['products_wholesale_price'],
            'stock_status':'instock' if int(convert['qty']) > 0  else 'outofstock',
            'tax_status': 'taxable',
        }

        self.import_product_data_connector(self.create_insert_query_connector("wc_product_meta_lookup", product_meta_lookup), True,
                                           convert['id'])
        thumbnail_id = False
        if convert['thumb_image']['url'] or convert['thumb_image']['path']:
            image_process = self.process_image_before_import(convert['thumb_image']['url'],convert['thumb_image']['path'])
            image_import_path = self.uploadImageConnector(image_process,
                                                          self.add_prefix_path(self.make_woocommerce_image_path(image_process['path'],
                                                          self.TYPE_PRODUCT), self._notice['target']['config']['image_product'].rstrip('/')))
            if image_import_path:
                product_image = self.remove_prefix_path(image_import_path,
                                                        self._notice['target']['config']['image_product'])
                image_details = 'TYPE_PRODUCT'
                # self.log(product_image,'product_image_product')
                # self.log(image_details,'image_details_product')
        ...
        # 	'ping_status': 'closed',
        # 	'post_name': str(product_id) + '-revision-v1',
        ...
        # 	'post_type': 'revision',
        # 	'comment_count': 0,
        # 	'guid': self._notice['target']['cart_url'] + '/2019/08/27/' + str(product_id) + '-revision-v1',
        # 	'post_excerpt': '',
        # 	'to_ping': '',
        # 	'pinged': '',
        # 	'post_content_filtered': ''
        # }
        # self.import_product_data_connector(self.create_insert_query_connector('posts', data_revision), True, convert['id'])
    ...
    def before_customer_import(self, convert, customer, customers_ext):
        return response_success()
    #TODO: CUSTOMER IMPORT
    def customer_import(self, convert, customer, customers_ext):
        customer_registered = ''
        for custom_info in customers_ext['data']['customers_info'] :
            if custom_info['customers_info_id'] == convert['id'] :
                customer_registered = custom_info['customers_info_date_account_created']
        users_data = {
        'user_login': convert['email'],
        'user_pass': convert['password'],
        'user_nicename':convert['first_name'] ,
        'user_email':convert['email'] ,
        'user_url':'http://localhost/wordpress' ,
        'user_registered': customer_registered ,
        'user_activation_key':'' ,
        'user_status':0 ,
        'display_name':convert['first_name'] + convert['last_name'] ,

        }
        customer_query = self.create_insert_query_connector('users', users_data)
        customer_id = self.import_customer_data_connector(customer_query, True, convert['id'])
        # self.log(convert,'convert_customer')
        # self.log(customer,'customer_customer')
        # self.log(customers_ext,'customers_ext_customer')
    ...
    def after_customer_import(self, customer_id, convert, customer, customers_ext):
        ...
        for custom_info in customers_ext['data']['customers_info'] :
            if custom_info['customers_info_id'] == convert['id'] :
                customer_update = custom_info['customers_info_date_account_last_modified']
                customer_registered = custom_info['customers_info_date_account_created']
        usemeta_data = {
            ...
            'last_update': customer_update,

        }
        for key, value in usemeta_data.items():
            usemeta_data_import = {
                'user_id': customer_id,
                'meta_key': key,
                'meta_value': value
            }
            usermeta_query = self.create_insert_query_connector('usermeta', usemeta_data_import)
            self.import_customer_data_connector(usermeta_query, True, convert['id'])


        customer_lookup_data = {

            'user_id': customer_id,
            'username': convert['email'],
            'first_name': convert['first_name'],
            'last_name': convert['last_name'],
            'email': convert['email'],
            'date_last_active': customer_update,
            'date_registered': customer_registered,
            'country': convert['address'][0]['country']['name'],
            'postcode': convert['address'][0]['postcode'],
            'city': convert['address'][0]['city'],
            'state': convert['address'][0]['state']['name'],

        }
        customer_lookup = self.create_insert_query_connector('wc_customer_lookup', customer_lookup_data)
        self.import_customer_data_connector(customer_lookup, True, convert['id'])
        return response_success()
    ...
    def order_import(self, convert, order, orders_ext):
        # self.log(convert,'convert_order')
        # self.log(order,'order_order')
        # self.log(orders_ext,'orders_ext_order')
        data = {
            'post_author': 1,
            'post_date': convert['created_at'] if convert['created_at'] and '0000-00-00' not in convert[
                'created_at'] else get_current_time(),
            'post_date_gmt': order['date_purchased'],
            'post_content': '',
            'post_title': 'Order &ndash;' + datetime.now().strftime('%B %d ,%Y @ %H:%M') ,
            'post_status': 'wc-completed',
            'comment_status': 'open',
            'ping_status': 'closed',
            'post_name': datetime.now().strftime( 'order-%b-%d-%Y-%H%M'  ),
            'post_modified': convert['created_at'] if convert['created_at'] and '0000-00-00' not in convert[
                'created_at'] else get_current_time(),
            'post_modified_gmt': convert['created_at'] if convert['created_at'] and '0000-00-00' not in convert[
                'created_at'] else get_current_time(),
            ...
            'comment_count': 0,
            'guid': 'http://localhost/wordpress/?post_type=shop_order',
            'post_excerpt': '',
            'to_ping': '',
            'pinged': '',
            'post_content_filtered': '',
        ...
        order_query = self.create_insert_query_connector('posts', data)
    ...
    def after_order_import(self, order_id, convert, order, orders_ext):
        postmeta_order= {
            ...
            '_billing_address_index': convert['customer']['middle_name'] + ',' + convert['billing_address']['company'] + ',' + convert['billing_address']['address_1'] + ',' + convert['billing_address']['city'] + ',' + convert['billing_address']['postcode'] + ',' + convert['customer']['email'] + ',' + convert['billing_address']['telephone'],
            '_prices_include_tax': 'no',
            '_order_version ': '4.8.0',
            '_order_total': convert['items'][0]['total'],
            '_order_tax': 0,
            ...
            '_billing_country': convert['billing_address']['telephone'],
            ...
            '_customer_user_agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.88 Safari/537.36',
            '_customer_ip_address': '::1',
            '_payment_method_title': convert['payment']['title'],
            '_payment_method': convert['payment']['method'],
            '_customer_user': 0,
            '_order_key': 'wc_order_ByMUqqwumFGL1',
            '_paid_date': order['date_purchased'],
            '_completed_date': order['date_purchased'],
            '_download_permissions_granted':'yes'
        }
        for key,value in postmeta_order.items():
            postmeta_order_data = {
                'post_id': order_id,
                'meta_key': key,
                'meta_value': value,
            ...
            postmeta_order_query = self.create_insert_query_connector('postmeta', postmeta_order_data)
            postmeta_order_id = self.import_order_data_connector(postmeta_order_query, True, convert['id'])

        for order_product in  orders_ext['data']['orders_products']:
            if convert['id'] == order_product['orders_id']:
                woocommerce_order_items_data = {
                    'order_item_name': order_product['products_name'],
                    'order_item_type': 'line_item',
                    'order_id': order_id,
                }
                order_item_query = self.create_insert_query_connector('woocommerce_order_items', woocommerce_order_items_data)
                order_item_id = self.import_order_data_connector(order_item_query, True, convert['id'])
                woocommerce_order_itemmeta_data = {
                    '_product_id': 0,
                    '_variation_id': 0,
                    '_qty': order_product['products_quantity'],
                    '_tax_class': '',
                    '_line_subtotal': order_product['products_price'],
                    '_line_subtotal_tax': '',
                    '_line_total': order_product['final_price'],
                    '_line_tax': order_product['products_tax'],
                    '_line_tax_data': 'a:2:{s:8:"subtotal";a:0:{}s:5:"total";a:0:{}}',
                    '_reduced_stock': order_product['products_quantity'],

                }
                for key , value in woocommerce_order_itemmeta_data.items():
                    import_iteamdata = {
                        'order_item_id': order_item_id,
                        'meta_key': key,
                        'meta_value':value
                    }
                    order_itemmeta_query = self.create_insert_query_connector('woocommerce_order_itemmeta', import_iteamdata)
                    order_itemmeta_id = self.import_order_data_connector(order_itemmeta_query, True, convert['id'])
        wc_order_stats_data = {
            'order_id': order_id,
            'parent_id': 0 ,
            'date_created': order['date_purchased'],
            'date_created_gmt': order['date_purchased'],
            'num_items_sold': convert['items'][0]['qty'],
            'total_sales': convert['items'][0]['total'],
            'tax_total': 0,
            'shipping_total': 0,
            'net_total': convert['items'][0]['total'],
            'returning_customer': 0,
            'status': 'wc-completed',
            'customer_id': 0,

        }
        order_stats_query = self.create_insert_query_connector('wc_order_stats', wc_order_stats_data)
        order_stats_id = self.import_order_data_connector(order_stats_query, True, convert['id'])
        return response_success()
